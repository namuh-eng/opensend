...
cd opensend
bun run setup
# Edit .env for production URLs, Google OAuth, SES, S3, and DNS.
docker compose up -d
```
...
`bun run setup` creates a complete `.env` with fresh local secrets for the app,
ingester, and scheduler. It generates `BETTER_AUTH_SECRET`,
`WEBHOOK_SECRET_ENCRYPTION_KEY`, `INTEGRATION_SECRET_ENCRYPTION_KEY`,
`INGESTER_JOB_TOKEN`, `INGESTER_INBOUND_TOKEN`, `TRACKING_SECRET`,
`UNSUBSCRIBE_SECRET`, `DKIM_ENCRYPTION_KEY`, `CRON_AUTH_TOKEN`, and the Compose
`POSTGRES_PASSWORD`. It prompts only for external provider values such as AWS,
Cloudflare, S3, and Google OAuth; blank answers are safe until those integrations
are used.

Production deploys should copy the generated values into a secrets manager or
platform secret store and inject them at runtime. Do not bake `.env` into images
...
| `BETTER_AUTH_SECRET` | Generated by `bun run setup`; random 32-byte hex string for session signing. The `.env.example` placeholder is allowed only for localhost evaluation; app and ingester startup checks reject it when the configured app URL is not localhost. |
| `BETTER_AUTH_TRUSTED_ORIGINS` | Comma-separated trusted browser origins. `bun run setup` defaults this to the app URL; production should include every dashboard origin. |
| `WEBHOOK_SECRET_ENCRYPTION_KEY` | Generated by `bun run setup`; random secret used to encrypt webhook signing secrets at rest. Docker Compose passes this into both the app and ingester. Production app and ingester startup fail when this is missing or too short. |
| `TRACKING_SECRET` / `UNSUBSCRIBE_SECRET` | Generated by `bun run setup`; HMAC secrets for open/click tracking and one-click unsubscribe tokens. Production app and ingester startup fail when either is missing or too short. |
| `DKIM_ENCRYPTION_KEY` | Generated by `bun run setup`; base64-encoded 32-byte AES key for DKIM private-key encryption. Production app and ingester startup fail when it is missing or malformed. |
...
| `INGESTER_JOB_TOKEN` | Generated by `bun run setup`; required 32+ character bearer token for `/jobs/*` endpoints when the scheduler/EventBridge invokes them over HTTP. Docker Compose refuses to start the ingester/scheduler without it. |
| `INGESTER_INBOUND_TOKEN` | Generated by `bun run setup`; bearer token for inbound event endpoints. Production ingester startup fails when it is missing or too short. |
