...
`.env.example` remains a placeholder reference, but `bun run setup` is the recommended path because it creates correctly shaped secrets for every boot preflight.
