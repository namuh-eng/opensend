import {
  authorizeDashboardOrApiKey,
  getServerSession,
  unauthorizedResponse,
} from "@/lib/api-auth";
import { requireFullAccessForApiKeyCaller } from "@/lib/api-key-permissions";
import { db } from "@/lib/db";
import { plans, subscriptions } from "@/lib/db/schema";
import { configurationSetService, dedicatedIpPoolRepo } from "@opensend/core";
import { eq } from "drizzle-orm";
import { NextResponse } from "next/server";
import { z } from "zod";

const createPoolSchema = z.object({
  name: z.string().min(1).max(255),
  ses_pool_name: z.string().min(1).max(255),
  scaling_mode: z.enum(["STANDARD", "MANAGED"]).optional().default("MANAGED"),
});

async function resolveUserId(req: Request): Promise<string | Response> {
  const auth = await authorizeDashboardOrApiKey(
    req.headers.get("authorization"),
  );
  if (!auth) return unauthorizedResponse();

  const permissionError = requireFullAccessForApiKeyCaller(auth);
  if (permissionError) return permissionError;

  if ("dashboard" in auth) {
    const session = await getServerSession();
    if (!session?.user?.id) return unauthorizedResponse();
    return session.user.id;
  }

  if ("userId" in auth && auth.userId) {
    return auth.userId;
  }

  return unauthorizedResponse();
}

async function getUserPlan(
  userId: string,
): Promise<{ dedicatedIpsEnabled: boolean; maxDedicatedIps: number } | null> {
  const sub = await db.query.subscriptions.findFirst({
    where: eq(subscriptions.userId, userId),
  });

  if (sub) {
    const plan = await db.query.plans.findFirst({
      where: eq(plans.id, sub.planId),
    });
    if (plan) {
      return {
        dedicatedIpsEnabled: plan.dedicatedIpsEnabled,
        maxDedicatedIps: plan.maxDedicatedIps,
      };
    }
  }

  // No subscription — treat as free plan (dedicated IPs disabled)
  return { dedicatedIpsEnabled: false, maxDedicatedIps: 0 };
}

export async function GET(req: Request) {
  const userIdOrResponse = await resolveUserId(req);
  if (userIdOrResponse instanceof Response) return userIdOrResponse;
  const userId = userIdOrResponse;

  try {
    const pools = await dedicatedIpPoolRepo.listForUser(userId);
    return NextResponse.json({
      object: "list",
      data: pools.map((p) => ({
        object: "dedicated_ip_pool",
        id: p.id,
        name: p.name,
        ses_pool_name: p.sesPoolName,
        scaling_mode: p.scalingMode,
        status: p.status,
        created_at: p.createdAt,
      })),
    });
  } catch (error) {
    console.error("Failed to list dedicated IP pools:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
  }
}

export async function POST(req: Request) {
  const userIdOrResponse = await resolveUserId(req);
  if (userIdOrResponse instanceof Response) return userIdOrResponse;
  const userId = userIdOrResponse;

  // Plan gating
  const planInfo = await getUserPlan(userId);
  if (!planInfo?.dedicatedIpsEnabled) {
    return NextResponse.json(
      {
        error: "Dedicated IP pools are not available on your current plan.",
        code: "plan_feature_unavailable",
      },
      { status: 403 },
    );
  }

  // Count quota
  const currentCount = await dedicatedIpPoolRepo.countForUser(userId);
  if (currentCount >= planInfo.maxDedicatedIps) {
    return NextResponse.json(
      {
        error: `Dedicated IP pool limit reached (${planInfo.maxDedicatedIps}).`,
        code: "quota_exceeded",
      },
      { status: 402 },
    );
  }

  let body: unknown;
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: "Invalid JSON body" }, { status: 400 });
  }

  const parsed = createPoolSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Validation failed", details: parsed.error.flatten() },
      { status: 422 },
    );
  }

  const {
    name,
    ses_pool_name: sesPoolName,
    scaling_mode: scalingMode,
  } = parsed.data;

  // Create in SES first
  try {
    await configurationSetService.createDedicatedIpPool({
      poolName: sesPoolName,
      scalingMode,
    });
  } catch (err) {
    console.error("Failed to create SES dedicated IP pool:", err);
    return NextResponse.json(
      { error: "Failed to provision dedicated IP pool in email provider." },
      { status: 502 },
    );
  }

  // Persist to DB
  try {
    const pool = await dedicatedIpPoolRepo.create({
      userId,
      name,
      sesPoolName,
      scalingMode,
      status: "active",
    });

    return NextResponse.json(
      {
        object: "dedicated_ip_pool",
        id: pool.id,
        name: pool.name,
        ses_pool_name: pool.sesPoolName,
        scaling_mode: pool.scalingMode,
        status: pool.status,
        created_at: pool.createdAt,
      },
      { status: 201 },
    );
  } catch (error) {
    console.error("Failed to persist dedicated IP pool:", error);
    // Best-effort cleanup of SES pool
    try {
      await configurationSetService.deleteDedicatedIpPool({
        poolName: sesPoolName,
      });
    } catch {
      // ignore
    }
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
  }
}
