...
function MobileDocsNav({
  ...
  return (
    <details className="mb-5 rounded-card border border-line bg-bg-card p-3 xl:hidden">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 rounded-md px-1 py-1 text-[13px] font-medium text-fg marker:hidden">
        <span>
          <span className="kicker block">Docs navigation</span>
          Browse documentation
        </span>
        <span className="rounded-full border border-line bg-white/[0.03] px-2 py-1 font-mono text-[11px] text-fg-3">
          {nav.reduce((count, section) => count + section.items.length, 0)}
        </span>
      </summary>
      <div className="docs-scrollbar mt-3 max-h-[68vh] overflow-y-auto pr-1">
        <a
          href="/docs"
        ...
        >
          ...
          <span className="mt-1 block text-[13px] font-medium">Start here</span>
        </a>
...
export function DocsShell({
  ...
  const layoutClassName = showRightRail
    ? "mx-auto grid w-full max-w-[1760px] gap-7 px-5 py-8 lg:px-8 xl:grid-cols-[286px_minmax(0,1fr)] 2xl:grid-cols-[286px_minmax(0,1fr)_240px]"
    : "mx-auto grid w-full max-w-[1500px] gap-7 px-5 py-8 lg:px-8 xl:grid-cols-[286px_minmax(0,1fr)]";
    ...
    <main className="landing-root min-h-screen">
      ...
      <div className="landing-content">
        <header className="sticky top-0 z-30 border-b border-line bg-bg/85 backdrop-blur-xl">
          <div className="mx-auto flex h-16 w-full max-w-[1760px] items-center justify-between px-5 lg:px-8">
            <a
              href="/docs"
            ...
            <nav className="hidden items-center gap-5 text-[13px] text-fg-3 md:flex">
              ...
              <a className="transition hover:text-fg" href="/docs/sdks">
                SDKs
              </a>
        ...
        <div className={layoutClassName}>
          <aside className="hidden xl:block">
            <div className="docs-scrollbar sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto pr-2">
              <a
                href="/docs"
                className={`mb-3 block rounded-card border px-3 py-3 transition ${activeSlug ? "border-line bg-white/[0.02] text-fg-2 hover:text-fg" : "border-accent/25 bg-accent-soft text-accent"}`}
              >
          ...
          <div className="min-w-0">
            <MobileDocsNav nav={nav} activeSlug={activeSlug} />
            {children}
          </div>
