...
function OutputSummary({
  ...
  return (
    <dl className="mt-3 grid grid-cols-1 gap-2 rounded-md border border-line bg-black/20 p-3 text-[12px] md:grid-cols-2">
      {rows.map((row) => (
        <div key={row.key}>
          <dt className="text-fg-4">{row.key}</dt>
          <dd className="break-words text-fg-2">{row.value}</dd>
        </div>
      ))}
    </dl>
  );
...
export function AutomationRunDetail({ automationId, runId }: Props) {
  ...
  if (loading) {
    return (
      <div className="py-16 text-center text-[14px] text-fg-2">
        Loading run...
      </div>
    );
  }
  ...
  if (notFound) {
    return (
      <div className="mx-auto max-w-3xl py-16 text-center">
        <h1 className="mb-2 text-[18px] font-semibold text-fg">
          Run not found
        </h1>
        <Link
          href={`/automations/${automationId}`}
          className="text-[13px] text-fg-2 underline hover:text-fg"
        >
          Back to automation
        </Link>
      </div>
    ...
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 flex items-start justify-between gap-3">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold text-fg">Run {run.id}</h1>
      ...
      <section className="mb-6 grid grid-cols-1 gap-3 md:grid-cols-2">
        {[
          ...
          <div key={label} className="rounded-lg border border-line p-4">
            ...
            <div className="mt-1 break-words text-[14px] text-fg">{value}</div>
          </div>
        ))}
      </section>
      ...
      <section className="rounded-lg border border-line">
        <div className="border-b border-line px-4 py-3">
          <h2 className="text-[14px] font-semibold text-fg">Step states</h2>
        ...
        {stepEntries.length === 0 ? (
          <p className="px-4 py-8 text-center text-[13px] text-fg-2">
            ...
            {stepEntries.map(([key, state]) => (
              <div key={key} className="p-4">
                <div className="mb-2 flex items-center justify-between gap-3">
                  <h3 className="text-[14px] font-semibold text-fg">{key}</h3>
                  <span className="rounded-full border border-line px-2 py-0.5 text-[12px] text-fg-2">
                    {capitalize(state.status)}
                  </span>
                </div>
                <dl className="grid grid-cols-1 gap-2 text-[13px] md:grid-cols-3">
                  <div>
                    <dt className="text-fg-4">Started</dt>
                    <dd className="text-fg-2">{formatDate(state.startedAt)}</dd>
                  </div>
                  <div>
                    <dt className="text-fg-4">Completed</dt>
                    <dd className="text-fg-2">
                      {formatDate(state.completedAt)}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-fg-4">Scheduled</dt>
                    <dd className="text-fg-2">
