...
export function createWebhookService({
  ...
  return {
    ...
    async updateWebhook(
      ...
      const [row] = await repository.update(id, buildUpdateData(input), userId);
      return row ? toWebhookListItem(row) : undefined;
    },
...
export class WebhookService {
  ...
  async list(options: { limit?: number; after?: string; userId: string }) {
  ...
  async update(id: string, input: UpdateWebhookInput, userId: string) {
    return await this.service.updateWebhook(id, input, userId);
  }
